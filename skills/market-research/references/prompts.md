...
4. Date every quote. After the source, include the post date in parentheses, formatted like
   (Jan 3, 2025). If the page doesn't show the date, check the Wayback Machine
   (web.archive.org) to find when it was first archived. Write (Date Unknown) if no date
   can be determined.

5. Verify every quote. Before saving a quote, re-visit the source URL and confirm the quote
   actually appears on the page. If it cannot be found, discard it and find a real
   replacement. Every quote must exist verbatim at its cited source.

6. After each theme, write a short "Pattern" note that summarizes the insight across its
   quotes — not just what they're saying, but what it reveals about how they think.

7. When all quotes are collected, write a 5–6 sentence persona sketch capturing the
   persona's mindset, daily challenges, and driving motivations, using only insights from
   your research.

8. Quality check: Confirm at least 20 quotes per category (minimum 80 total).
...
IDENTITY
  Theme 1: [theme name]
    "Quote one." — [source] (Mon D, YYYY)
    "Quote two." — [source] (Date Unknown)
    Pattern: [insight]
  Theme 2: [theme name]
    ...
